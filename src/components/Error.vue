...
export default {
  ...
  computed: {
    ...mapState( {
      ...
      currentLang: state => state.error.currentLang
    } ),
    code( ) {
      return this.responseCode || '';
    },
    message( ) {
      const messageI18n = this.$t( this.code );
      const message = messageI18n === this.code && this.responseMessage ? this.responseMessage : messageI18n;
